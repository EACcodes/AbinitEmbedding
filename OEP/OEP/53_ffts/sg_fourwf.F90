...
! Problem is that the code does not set max1i/max2i when
! option == 3, but uses them below, so I try to remove the if
! if(option/=3)then
...
! end if
 ...
 md2=max(md2i,md2o)
 md3=max(md3i,md3o)
! this seems buggy for option 3, as m2i is then not initialized!
! this cost me three days to find! FL
! md2proc=(m2i-1)/mpi_enreg%nproc_fft+1
 md2proc=(md2-1)/mpi_enreg%nproc_fft+1
