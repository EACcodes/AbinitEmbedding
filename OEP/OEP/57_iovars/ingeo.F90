...
!!
!! CHILDREN
!!      atmdata,chkorthsy,fillcell,gensymshub,gensymshub4,gensymspgr,ingeo_img
!!      ingeobld,intagm,mati3inv,metric,mkrdim,randomcellpos,symanal,symatm
!!      symfind,symlatt,symrelrot,symzat,wrtout,xcart2xred,xred2xcart
!!
!! SOURCE
...
subroutine ingeo (acell,amu,dtset,bravais,&  !!HONG
...
!arrays
 ...
 real(dp),intent(inout) :: spinat(3,natom)  !!HONG
 real(dp),intent(out) :: acell(3),amu(ntypat),genafm(3),mixalch(npspalch,ntypalch)
 real(dp),intent(out) :: rprim(3,3),tnons(3,msym)
...
!scalars
 ...
 real(dp) :: amu_default,a2,aa,cc,cosang,ucvol,sumalch,rcov
...
!arrays
 ...
 real(dp) :: angdeg(3), field_xred(3),gmet(3,3),gprimd(3,3),rmet(3,3),rcm(3)  !!HONG
...
!1) set up unit cell : acell, rprim and rprimd ---------------------
 ...
 call ingeo_img(acell,iimage,jdtset,lenstr,nimage,3,string,token,tacell,'LEN')
...
!end if
 ...
 call ingeo_img(scalecart,iimage,jdtset,lenstr,nimage,3,string,token,tscalecart,'LEN')
!if(nimage/=1 .and. iimage/=1)then
...
!Check that input length scales acell(3) are > 0
 do mu=1,3
   if(acell(mu)<=0.0_dp) then
     write(message, '(a,i5,a, 1p,e14.6,a,a,a,a)' )&
...
!Initialize rprim, or read the angles
 ...
 call ingeo_img(rprim,iimage,jdtset,lenstr,nimage,3,3,string,token,trprim,'DPR')
!if(nimage/=1 .and. iimage/=1)then
!token = 'rprim_lastimg'
...
!If none of the rprim were read ...
 if(trprim==0)then
   ...
   angdeg(:)=dprarr(1:3)
   call ingeo_img(angdeg,iimage,jdtset,lenstr,nimage,3,string,token,tangdeg,'DPR')
!  if(nimage/=1 .and. iimage/=1)then
...
!If not read, try the CML data
 ...
 do iatom=1,natrd
   if(typat_read(iatom)<1 .or. typat_read(iatom)>ntypat )then
     write(message,'(a,i4,a,i3,a,a,a,i3,a,a,a)')&
...
!end if
 ...
 call ingeo_img(xred_read,iimage,jdtset,lenstr,nimage,3,natrd,string,token,txred,'DPR')
!if(nimage/=1 .and. iimage/=1)then
!token = 'xred_lastimg'
...
!ENDDEBUG
 ...
 call ingeo_img(xangst_read,iimage,jdtset,lenstr,nimage,3,natrd,string,token,txangst,'DPR')
!if(nimage/=1 .and. iimage/=1)then
...
!end if
 ...
 token = 'xcart'
 call intagm(dprarr,intarr,jdtset,marr,3*natrd,string(1:lenstr),token,txcart,'LEN')
 if(txcart==1 .and. txrandom==0)xcart_read(:,1:natrd) = reshape( dprarr(1:3*natrd) , (/3,natrd/) )
 call ingeo_img(xcart_read,iimage,jdtset,lenstr,nimage,3,natrd,string,token,txcart,'LEN')
!if(nimage/=1 .and. iimage/=1)then
!token = 'xcart_lastimg'
...
!Check that nsym is not bigger than msym
 if (nsym>msym) then
   write(message, '(a,i4,a,i4,a,a,a,a,a)' )&
   ...
   if(symmorphi==0)then
     do isym=1,nsym
       if(sum(tnons(:,isym)**2)>tol6)then
         write(message, '(5a,i2,a,3f8.4,3a)' )&
...
!At this stage, nsym might still contain the default 0, msym contains the default 192.
...
&       '  Action : change one of the input variables jellslab or nsym or spgroup in your input file.'
     ...
     if(spgroup/=0 .and. nsym/=0)then
       write(message, '(a,i4,a,a,i4,a,a,a,a,a,a,a,a)' )&
...
!      Treat the case of magnetic groups
       ...
       if(tspgroupma/=0 .and. tgenafm/=0)then
         write(message, '(a,i4,a,a,3es9.2,a,a,a,a,a,a,a,a)' )&
...
!    of atoms to be read must equal the total number of atoms.
     if(natrd/=natom)then
       ...
       write(message, '(a,i4,a,a,i4,a,a,a,a,a,a,a,a,a)' )&
...
!!================= chen hack in ==========================
!! We enforce moving atoms to the center of the box, even if
!! icoulomb is 0
!!
...
!  Checks the validity of natfix
   if (natfix<0 .or. natfix>natom) then
     write(message, '(a,a,a,i4,a,i4,a,a,a)' )&
...
!  Read iatfix
   if(idir==0)then
     token = 'iatfix'
     call intagm(dprarr,intarr,jdtset,marr,natfix,string(1:lenstr),token,tread,'INT')
   else if(idir==1)then
     token = 'iatfixx'
     call intagm(dprarr,intarr,jdtset,marr,natfix,string(1:lenstr),token,tread,'INT')
   else if(idir==2)then
...
!  ENDDEBUG
 ...
 call ingeo_img(vel,iimage,jdtset,lenstr,nimage,3,natom,string,token,tread,'DPR')
!if(nimage/=1 .and. iimage/=1)then
!token = 'vel_lastimg'
!call intagm(dprarr,intarr,jdtset,marr,3*natom,string(1:lenstr),token,tread,'DPR')
!if(tread==1)vel(:,:)=vel(:,:)+alpha*(reshape( dprarr(1:3*natom) , (/3,natom/) )-vel(:,:))
...
&       '  Action : check the content of the input variable mixalch.'
   ...
   call ingeo_img(mixalch,iimage,jdtset,lenstr,nimage,npspalch,ntypalch,string,token,tread,'DPR')
 end if
...
!Find the default mass
 ...
 do ipsp=1,npsp
   call atmdata(amu_default,rcov,symbol,znucl(ipsp))
...
!When the pseudo-atom is alchemical, must make mixing
 ...
 call ingeo_img(amu,iimage,jdtset,lenstr,nimage,ntypat,string,token,tread,'DPR')
...
!write(std_out,*)' ingeo : mixalch=',mixalch(1:npspalch,1:ntypalch)
!do isym=1,nsym
!write(std_out,'(i2,9i3,3f8.4,i3)' )isym,symrel(:,:,isym),tnons(:,isym),symafm(isym)
!end do
!ENDDEBUG
