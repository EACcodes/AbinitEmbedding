...
! pbar(3) and dbar(3) are reduced polarization and displacement field, defined by Eq.(27) and (29) Nat. Phys. suppl. (2009)
 ...
 real(dp) ::   cgW, & 
               ...
               eold1_c,eold2_c,wtol,ektol,stoptol,& 
               tmp1,tmp2,tmp3, number_e, & 
               vtmp(dtset%nfft,2), &
               vks(dtset%nfft,dtset%nspden), &
               vinitial(dtset%nfft,dtset%nspden), &
               vdiff(dtset%nfft,dtset%nspden)
 ! variables added by Jin
 real(dp) :: best_cgW, best_ke, best_res, res
 character(len=60) :: fname
 ! end
 real(dp) :: ke_firstOrder

! penalty funciton related
 real(dp) :: penfun, penfun2, penLambda, lapvfg(dtset%nfft), lapvfg2(dtset%nfft)
...
!  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MAIN LOOP START  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< !
   ...
   call load_param(bLoadDen,bLoadVr,maxcount,bAllowsym,bSpin,wtol,stoptol, structure,ektol,penLambda)
...
!!     last_x   = vtrial(:,1)
   ...
   write (message, '(A,es12.4,a,es12.4,a,es12.4)') & 
    'ektol = ',ektol,' wtol=',wtol, ' stoptol=', stoptol
   call c_wrtlog(message)
...
! Jin's change.....
!     if (restart==.false. .and. tigger_mode == .false.) then
   if ( bLoadDen == 1 ) then
     ! Jin, no matter what, prepare vtrial if reference density is loaded
       if(bSpin==1) then
         rhor(:,1) = ref_rhor(:);
       else
...
&      taug=taug,taur=taur,vxctau=vxctau)
       ...
       call c_wrtlog("set vtrial = vhart + vpsp.")
       ! Jin, output vks[rhor_ref]
       vks(:,1) = vtrial(:,1)
   endif
   ...
   if (bLoadVr == 1) then
     ...
     call c_wrtlog("proceed to do invert-KS with it.")
   endif
   ...
   if (bLoadVr == 2) then
     ...
     if(bSpin==2) then
       ...
       read (1112, *) vtrial(:,2);
       close(1112);
       vtrial(:,2) = vtrial(:,2) + vks(:,2)
     endif

     call c_wrtlog( "Load in refpot.in and set initial guess to refpot + vks")
     call c_wrtlog("proceed to do invert-KS with it.")
   endif

! at this point, save the initial guess to vinitial 
   vinitial = vtrial

     if (bLoadDen == 0 .and.  bLoadVr == 0) then
...
!          write(std_out,*) 'after calling vtorho'
!          write(std_out,*) 'max/min of rhor:', maxval(rhor(:,1)),minval(rhor(:,1))
...
&        electronpositron=electronpositron,taug=taug,taur=taur,vxctau=vxctau)
       write(std_out, *) 'max/min of vtrial:', maxval(vtrial(:,1)), minval(vtrial(:,1))
       if(bSpin==2) then
         write(std_out, *) 'max/min of vtrial2:', maxval(vtrial(:,2)), minval(vtrial(:,2))
       endif
...
&               energies%e_entropy +  energies%e_ewald + energies%e_nonlocalpsp
       ...
       !=================== calculate penalty ================
       if ( bLoadVr == 2 ) then
           vdiff(:,1) = vtrial(:,1)-vinitial(:,1)
       else
           vdiff(:,1) = vtrial(:,1)-vks(:,1)
       endif
       call laplacian(gprimd,mpi_enreg,nfftf,1,ngfftf,dtset%paral_kgb,rdfuncr=vdiff(:,1),laplacerdfuncr=lapvfg)
       if(bSpin==2) then
           if ( bLoadVr == 2 ) then
              vdiff(:,2) = vtrial(:,2)-vinitial(:,2)
          else
              vdiff(:,2) = vtrial(:,2)-vks(:,2)
          endif
          call laplacian(gprimd,mpi_enreg,nfftf,1,ngfftf,dtset%paral_kgb,rdfuncr=vdiff(:,2),laplacerdfuncr=lapvfg2)
...
!         penfun = -penLambda*dot_product(vtrial(:,1),lapvfg)*ucvol/dble(nfftf)
         penfun = -penLambda*dot_product(vdiff(:,1),lapvfg)*ucvol/dble(nfftf)
          ! added by Jin
          res = -(dot_product(vtrial(:,1),rhor(:,1)-ref_rhor)*ucvol/dble(nfftf))
       endif
...
!                  -penLambda*dot_product(vtrial(:,2),lapvfg2)*ucvol/dble(nfftf)
          penfun = -penLambda*dot_product(vdiff(:,1),lapvfg)*ucvol/dble(nfftf) &
                   -penLambda*dot_product(vdiff(:,2),lapvfg2)*ucvol/dble(nfftf)
          penfun2 = -penLambda*dot_product(vdiff(:,2),lapvfg2)*ucvol/dble(nfftf)
          ! added by Jin
          res = dot_product(vtrial(:,1),rhor(:,2)-ref_rhor)*ucvol/dble(nfftf) + &
                dot_product(vtrial(:,2),rhor(:,1)-rhor(:,2)-ref_rhor2)*ucvol/dble(nfftf)
       endif
       cgW    = cgW + penfun
...
!!   Here I obtain the Ts by subtracting the NLPS and entropy energies from cgW
       ...
       !! Jin's checking code...
!       if (bSpin == 1) then
!          open(unit=999, file='first_rhor')
!          rewind(999)
!          write(999,'(e22.15)') rhor(:,1)
!          close(999)
!!          stop
!       endif
...
!          open(unit=999, file='first_rhor')
!          rewind(999)
!          write(999,'(e22.15)') rhor(:,2)
!          close(999)

!          open(unit=999, file='first_rhor2')
!          rewind(999)
!          write(999,'(e22.15)') rhor(:,1)-rhor(:,2)
!          close(999)
!          stop
!       endif
...
!! end of debug       
       ...
       IF ( task(1:5) .EQ. 'NEW_X')  THEN
         ...
         nBFGS   = nBFGS + 1
         ! back up everything
!         if  ( abs(res) <= best_res ) then   ! current vtrial gives out better density
!             best_res = abs(res)
!             open(unit=1, file='best_rep.dat')
!             rewind(1)
!             write(1,'(e22.14)') rhor(:,1)
!             close(1)
!             open(unit=1, file='best_rhoV.dat')
!             rewind(1)
!             write(1,'(e22.14)') vtrial(:,1)
!             write(1,'(a, e22.14)' ) 'best_res = ', best_res
!             close(1)
!         endif

!         if  ( cgW <= best_cgW ) then   ! current vtrial gives out lowest cgW value
!             best_cgW = cgW
!             open(unit=1, file='best_WV.dat')
!             rewind(1)
!             write(1,'(e22.14)') vtrial(:,1)
!             write(1,'(a, e22.14)' ) 'best_cgW = ', best_cgW
!             close(1)
!         endif

!         if  ( ke_firstOrder >= best_ke ) then   ! current vtrial gives out highet ke value
!             best_ke = ke_firstOrder
!             open(unit=1, file='best_keV.dat')
!             rewind(1)
!             write(1,'(e22.14)') vtrial(:,1)
!             write(1,'(a, e22.14)' ) 'best_ke = ', best_ke
!             close(1)
!         endif
!         if ( myicount <= 9 ) then
!            write(fname,'(a,i1)') 'vtrial',myicount
!         elseif( myicount >=10 .and. myicount <= 99 ) then
!            write(fname,'(a,i2)') 'vtrial',myicount   
!         elseif (myicount >=100 .and. myicount <= 999) then
!            write(fname,'(a,i3)') 'vtrial',myicount
!         endif
!         open(unit=1, file=fname)
!         write(1, '(e22.15)') vtrial(:,1)
!         write(1,*) 'cgW = ', cgW
!         write(1,*) 'ke_firstOrder = ', ke_firstOrder
!         write(1,*) 'res = ', res 
!         close(1)
            last_x = vtrial(:,1)
!            open(unit=1, file='tmp_vtrial')
!            rewind(1)
!            write(1,'(e22.15)') last_x
!            close(1)
!         if (bSpin==1) then
!            open(unit=1, file='tmp_rhor')
!            rewind(1)
!            write(1,'(e22.15)') rhor(:,2)
!            close(1)
!         endif

!         if (bSpin==2) then
!            last_x2 = vtrial(:,2)
!            open(unit=1, file='tmp_vtrial2')
!            rewind(1)
!            write(1,'(e22.15)') last_x2
!            close(1)
!            open(unit=1, file='tmp_rhor')
!            rewind(1)
!            write(1,'(e22.15)') rhor(:,2)
!            close(1)
!            open(unit=1, file='tmp_rhor2')
!            rewind(1)
!            write(1,'(e22.15)') (rhor(:,1)-rhor(:,2))
!            close(1)
!         endif
         ...
         if (bSpin == 2) then
            write(message,'(a,I4,a,g22.14,a,ES12.4,a)') &
                  '(scfcv) iter:',nBFGS,' cgW(with penalty)=',cgW,' penalty1=', penfun-penfun2,' (new iter)'
            call c_wrtlog(message)
            write(message,'(a,I4,a,g22.14,a,ES12.4,a)') &
                  '(scfcv) iter:',nBFGS,' cgW(with penalty)=',cgW,' penalty2=',penfun2,' (new iter)'
            call c_wrtlog(message)
         endif
         ...
         if (myicount > 5) then
           ...
           if ( maxval(abs(ref_rhor - rhor(:,1))) < stoptol ) then
             write(message,'(a,es12.4,a)') &
               '(converged) The maximum density difference is less than ', stoptol,' bohr^-3'
             call c_wrtlog(message)
             write(message,'(a,es20.12,a)') &
              '(converged) final etotal = ',etotal,' hartree'
             call c_wrtlog(message)
       ...
       elseif (task(1:2) .EQ. 'FG') then
         ...
         if (nline>0) then 
           ...
           if (bSpin==2) then
              write(message,'(a,I3,a,g22.14,a,ES12.4,a)') &
               '(scfcv) nline:',nline,' cgW(with penalty)=',cgW,' penalty1=', penfun-penfun2,' '
              call c_wrtlog(message)
              write(message,'(a,I3,a,g22.14,a,ES12.4,a)') &
               '(scfcv) nline:',nline,' cgW(with penalty)=',cgW,' penalty2=', penfun2,' '
              call c_wrtlog(message)
           endif
...
!!=======================       
   if (tigger_mode) then 
     ...
     if (mpi_enreg%me==0) then
       open(file='oep_results.dat',action='write',unit=111)
       write(111, '(e22.15)')ke_firstOrder
       write(111, '(e22.15)')energies%e_nonlocalpsp
       write(111, '(e22.15)')energies%e_entropy
       write(111, '(e22.15)')-vtrial(:,1)+energies%e_fermie

       call c_wrtlog('[tigger_mode] tspot=(-vtrial+fermie) dumpped to ts.pot')
       close(111)

       if ( bSpin==2 ) then 
           open(file='oep_results2.dat',action='write',unit=111)
           write(111, '(e22.15)')ke_firstOrder
           write(111, '(e22.15)')energies%e_nonlocalpsp
           write(111, '(e22.15)')energies%e_entropy
           write(111, '(e22.15)')-vtrial(:,2)+energies%e_fermie
           close(111)
      endif
...
! Jin
   ...
   if (mpi_enreg%me==0) then
       ...
       write(111, '(e22.15)')-vtrial(:,1)+energies%e_fermie
       close(111)
       write(msg,*) 'create the oep_results.dat file...'
       call c_wrtlog(msg)
       ...
       if ( bSpin==2 ) then
          ...
          write(111, '(e22.15)')-vtrial(:,2)+energies%e_fermie
          close(111)
          write(msg,*) 'create the oep_results2.dat file...'
          call c_wrtlog(msg)
       endif

       open(unit=1, file='vks.dat')
       rewind(1)
       write(1,'(e25.15)') vks(:,1)
       close(1)
       write(msg,*) 'create the vks.dat file...'
       call c_wrtlog(msg)
  
       if ( bSpin==2 ) then
          open(unit=1, file='vks2.dat')
          rewind(1)
          write(1,'(e25.15)') vks(:,2)
          close(1)
          write(msg,*) 'create the vks2.dat file...'
          call c_wrtlog(msg)
       endif

       open(unit=1, file='voep.dat')
       rewind(1)
       write(1,'(e25.15)') vtrial(:,1)
       close(1)
       write(msg,*) 'create the voep.dat file...'
       call c_wrtlog(msg)
      
       if ( bSpin==2 ) then
          open(unit=1, file='voep2.dat')
          rewind(1)
          write(1,'(e25.15)') vtrial(:,2)
          close(1)
          write(msg,*) 'create the voep2.dat file...'
          call c_wrtlog(msg)
       endif
...
subroutine load_param(bLoadDen,bLoadVr,maxcount,bAllowsym,bSpin,wtol,stoptol,structure,ektol,penLambda)
   implicit none
   character(len=500) :: structure,msg
   integer :: bLoadDen, bLoadVr, maxcount, bAllowsym, bSpin, itmp1
   real*8  :: wtol,stoptol, ektol,penLambda,tmp1
   ...
   read (1111,*) stoptol, msg     ! if max_diff less than stoltol, program converges
