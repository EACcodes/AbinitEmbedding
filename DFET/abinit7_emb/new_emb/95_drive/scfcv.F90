...
!============================================================
! load rhor from refden.in as initial guess
!============================================================
!   open(file='refden.in',unit=111,action='read')
!   read(111,*) rhor(:,1)
!   close(111)
...
!============================================================
! load extpot (embedding potential) from embpot.dat
!============================================================
    write(std_out, *) ' loading embpot.dat file...'
    open(unit=1111, action="read", file='embpot.dat')
    allocate(extpot(nfftf))
    extpot=0.0
    rewind(1111)
    read(1111,*) extpot
    close(1111)
    write(std_out, *) ' empot.dat file loaded'

!============================================================
! add extpot to vtrial and vpsp(ionic local potential)
! for the current version, PAW method is not considered
!============================================================
       vpsp = vpsp + extpot
...
!----------------------------------------------------------------------
 ...
 call timab(248,1,tsec)
   ...
   if ((mpi_enreg%nproc>1 .and. mpi_enreg%me==0) .or. mpi_enreg%nproc==1) then
!write(*,*) "I am writing", mpi_enreg%nproc, mpi_enreg%me

!============================================================
! Dump the results to rhor.dat
!============================================================
   open(file='rhor.dat',unit=111,action='write')
   write(111,'(e22.15)') rhor(:,1)
   close(111)

   if ( rhor(1,2) /= 0 ) then
      open(file='rhor1.dat',unit=111,action='write')
      write(111,'(e22.15)') rhor(:,1) - rhor(:,2)
      close(111)

      open(file='rhor2.dat',unit=111,action='write')
      write(111,'(e22.15)') rhor(:,2)
      close(111)
   endif

   open(file='kinetic.dat',unit=111,action='write')
   write(111,'(e22.15)')energies%e_kinetic
   write(111,'(e22.15)')energies%e_nonlocalpsp
   write(111,'(e22.15)')energies%e_entropy
   write(111,'(e22.15)')-vtrial(:,1)+energies%e_fermie
   close(111)

   open(file='vext.dat',unit=111,action='write')
   write(111,'(e22.15)') vpsp-extpot
   close(111)
 endif
