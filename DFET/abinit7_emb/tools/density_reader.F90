...
program tester
  ...
  call read_header(31, header) rho(:,:)

  open(31,file="abinit_example_density.DEN", status="OLD", form="UNFORMATTED")
  
  call read_header(31, header)

  allocate(rho(header%cplex*header%ngfft(1)*header%ngfft(2)*header%ngfft(3),header%nspden))
...
!  rho = ....
  ...
  call write_density_file("abinit_example_DEN",rho,header);
