...
!C
!C    Changes by chen: A fourth method for gradient calculation
!C 
...
!====================== chen =================
      double precision yk(N),dTy,norm_y,cg_eta,norm_dk,norm_gk
!=============== end of modification =========
...
!C
  75  NFUN= NFUN + NFEV
      ...
      ELSE
        ...
        !========== chen =================
        ! Hager && Zhang, SIAM J. OPTIM  (2005)
        !
        IF (METHOD==4) THEN
          yk     = g-gold
          dTy    = dot_product(d,yk)
          norm_y = sqrt(dot_product(yk,yk))
          beta   = 1.d0/dTy * dot_product(yk-2.d0*d*norm_y**2/dTy,g)
          
          cg_eta = 0.d0
          norm_dk= sqrt(dot_product(d,d))
          norm_gk= sqrt(dot_product(gold,gold))
          beta   = max(beta,-1.d0/(norm_dk*min(cg_eta,norm_gk)))
        ENDIF
        !============ end of code ========
      ENDIF
!C
